 ...
 *
 * Version: $Id: basecommands.sp 1124 2007-07-15 17:22:47Z faluco $
 */
...
public Action:Command_BanIp(client, args)
{
	...
	if (act < Plugin_Handled)
	{
		...
		ReplyToCommand(client, "[SM] Ban has been added.");
	}
...
public Action:Command_Unban(client, args)
{
	...
	if (act < Plugin_Handled)
	{
		...
		ReplyToCommand(client, "[SM] Removed bans matching filter: %s", arg);
	}
...
public Action:Command_Ban(client, args)
{
	...
	if (act < Plugin_Handled)
	{
		if (!time)
		{
			if (reason[0] == '\0')
			{
				ShowActivity(client, "Permanently banned player \"%s\".", arg);
			} else {
				ShowActivity(client, "Banned player %s for %i minutes (reason: %s).", arg, reason);
			}
		} else {
			if (reason[0] == '\0')
			{
				ShowActivity(client, "Banned player \"%s\" for %i minutes.", arg, time);
			} else {
				ShowActivity(client, "Banned player %s for %i minutes (reason: %s).", arg, time, reason);
			}
		}
...
public Action:Command_ExecCfg(client, args)
{
	...
	if (!FileExists(path))
	{
		ReplyToCommand(client, "[SM] Executed config \"%s\".",  path[4]);
		return Plugin_Handled;
	}
...
public Action:Command_Cvar(client, args)
{
	...
	if (!allowed)
	{
		ReplyToCommand(client, "[SM] You do not have access to this cvar.");
		return Plugin_Handled;
	}
	...
	if (args < 2)
	{
		...
		ReplyToCommand(client, "[SM] Value of cvar \"%s}\": \"%s\"", cvarname, value);
		return Plugin_Handled;
	}
	...
	{
		ShowActivity(client, "Changed cvar \"%s\" to \"%s\".", cvarname, value);
	} else {
		ReplyToCommand(client, "[SM] Value of cvar \"%s\": \"%s\"", cvarname, value);
	}
